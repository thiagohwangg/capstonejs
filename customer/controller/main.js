...
renderSP = (result, resultdata) => {
  ...
  for (let index = 0; index < result; index++) {
    ...
    htmlImg += `<div class="col-12 col-sm-6 col-md-4 col-lg-3" style="
        ...
        margin-bottom: 10px;
        margin-top: 15px;">
        <button style="border-radius:20px" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="hoverPrd('${
          prd.name
        }','${prd.screen}','${prd.backCamera}','${prd.frontCamera}','${
      prd.desc
    }')">
        <image src="${
          prd.image
        }" style="width:100%; border-radius:15px;max-height: 50%"/>
        </button>
        </div>
        ...
        <div>
        <span style="color:#FF1493;font-size:20px">${Number(
          prd.price
        ).toLocaleString()} VNĐ</span>
        </div>
        </div>
        <button id="btnThemGioHang" class="btn btn-success" style="width:60%;margin-top:10px" onclick="buyItem(${
          prd.id
...
renderCart = () => {
  ...
  for (let i = 0; i < cartList.arrSP.length; i++) {
    ...
    htmlContent += `<div class="col-4" style="margin-bottom: 20px;max-height: 100%;">
        ...
        <p class="cart__text">
        <span style="font-weight:700; color:#CC99FF">Màn hình: </span>${
          b.screen
        }</br>
        <span style="font-weight:700; color:#CC99FF">Camera sau: </span>${
          b.backCamera
        }</br>
        <span style="font-weight:700; color:#CC99FF">Camera trước: </span>${
          b.frontCamera
        }</br>
        </p> 
        </div>
        ...
        <span class="bg" onclick="giamSL('${
          b.maSP
        }')">-</span> <span class="numberCart">${
      b.quantity
    }</span> <span class="bg" onclick="tangSL('${b.maSP}')">+</span>
        </p>
        </div>
        ...
        max-width: 150px;margin-bottom:15px;" onclick="removeItem(${
          b.maSP
        })">Remove</button>
        </div>
        `;
